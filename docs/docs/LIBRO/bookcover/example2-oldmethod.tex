\documentclass[markcolor=black,spinewidth=15mm,bgtikznodes]{bookcover}
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage[english]{babel}
\usepackage{url}
\usepackage{graphicx}
\usepackage{contour,lipsum}
\definecolor{amiyellow}{cmyk}{0,0,.5,0}
\begin{document}

% % % The outside of the book cover 
% Yellow triangle on the back cover by tikz code
% \setbookcover{bgtikz}{back}{
%     \fill[color=amiyellow](.5,.5)--(17.5,24.5)--(17.5,0)--(.5,0)--cycle;}

% Yellow triangle on the front cover by tikz code
% \setbookcover{bgtikz}{front}{
%     \fill[color=amiyellow](0,0)--(0,24.5)--(17,.5)--(17,0)--cycle;}

% Yellow background color on the spine
% \setbookcover{bgcolor}{spine}{color=amiyellow}

% \setbookcover{fgfirst}{above front}{
%     \color{blue} Protopopov 1996--1999 | \textsc{Tratado}}

% Remark
\setbookcover{fgfirst}{above front}{
    \color{red}Protopopov 1996--1999 | \textsc{Tratado Sobre el Amor} %book cover -- outside
    }

% Text on the spine
\setbookcover{fgfirst}{spine}{
    \vfill
    \centering
    \rotatebox[origin=c]{90}{
        \footnotesize\bfseries 
        TRATADO SOBRE EL AMOR | Anatoliy Protopopov}
    \vfill}
    
\setbookcover{fgfirst}{back}{
    \centering
    \vspace{20mm}
    \parbox{110mm}{%\color{white}
    \lipsum[2-4]
    }}

% Text and pictures (summa.pdf, ekflogo.pdf) on the front cover
\setbookcover{fgfirst}{front}{
    \vspace{20.5mm}
    \centering
    {\huge\bfseries \textsc{Tratado} \textsc{Sobre} %\\ 
    \textsc{El Amor} \\
    {\large \textsc{como lo entiende una persona terriblemente aburrida}}\\
    [13mm]}
    {\LARGE\bfseries Anatoliy Protopopov
    }\\[10mm]
    \includegraphics[width=1\textwidth]{./figures/1-peacock-dean-nahum-unsplash-4226.jpg}\\[5mm]
%     {\large Anatoliy Protopopov
%     {\huge Anatoliy Protopopov
    %COMMISSIO REDACTORIUM
%     }
    \\[3mm]
    \parbox{123mm}{
        \centering
  % AAVV
}
    \vfill
    {\large\bfseries ediciones intratables} \\ \vfill
    \includegraphics[height=20mm]{./figures/logo.png}
    \vfill
    \vfill}

% Making the outside book cover 
\makebookcover
\end{document}


% The inside of the book cover 

% Remark
\setbookcover{fgfirst}{above front}{
    \color{red}\textsc{Annales Mathematicae et Informaticae} book cover -- inside}

% Text on the front cover (it is back of the inside book cover!)
\setbookcover{fgfirst}{front}{
    \vspace{27mm}
    \begin{center}
        \bfseries
        ANNALES MATHEMATICAE ET INFORMATICAE\\[3mm]
        International journal for mathematics and computer science\\[3mm]
        Referred by\\
        Zentralblatt für Mathematik\\
        and\\
        Mathematical Reviews\\
    \end{center}
    \bigskip
    \begin{center}
        \parbox{126mm}{
            The journal of the Institute of Mathematics and Informatics of Eszterházy Károly University is open for scientific publications in mathematics and computer science, where the field of number theory, group theory, constructive and computer aided geometry as well as theoretical and practical aspects of programming languages receive particular emphasis. Methodological papers are also welcome. Papers submitted to the journal should be written in English. Only new and unpublished material can be accepted.}
    \end{center}}

% Making the inside book cover 
\makebookcover

\end{document} 